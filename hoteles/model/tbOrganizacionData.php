...
{ 
  ...
  { // metodo add, complementar
    $str = "insert into " . self::$tablename . "(nitDni, nroPisos, desGeneral1, desGeneral2, idDepartamento, idCiudad, nombOrg, dirbOrg, noTelf1, noTelf2, emailOrg, password, nroHabXpiso, aforoPersonas, rutaImagen, status, admon, admontel)";
    $str .= " values ('$this->nitDni', $this->nroPisos, '$this->desGeneral1', '$this->desGeneral2', $this->idDepartamento, $this->idCiudad, '$this->nombOrg', '$this->dirbOrg', '$this->noTelf1', '$this->noTelf2', '$this->emailOrg', $this->password, $this->nroHabXpiso, $this->aforoPersonas, '$this->rutaImagen', $this->status, '$this->admon', '$this->admontel');";
  ...
  {  
    $str="";
    $str = "Call spnew_tbOrganizacion('$this->nitDni', $this->nroPisos, '$this->IdDescripcionTr', $this->IdCiudad, '$this->nombOrg', '$this->dirbOrg', '$this->noTelf1', '$this->noTelf2' '$this->emailOrg', , '$this->password', '$this->idTipoTr', '$this->idredsocialtr', $this->nroHabXpiso, $this->aforoPersonas, '$this->idServicioTr', '$this->idInstalacionTr', '$this->idAccesibilidadTr', '$this->rutaImagen', $this->status);";
  ...
  public static function oneTbOrganizacion($id)
  {
    ...
    while ($rx = $query[0]->fetch_array()) {
      ...
      $array1[$cn]->password = $rx['password'];
  ...
  public static function pageTbOrganizacion($inicio, $pag, $rgtro)
  {
    ...
    return $array1;
  	...
  	public static function getByMail($mail){
		$sql = "select * from ".self::$tablename." where emailOrg=\"$mail\"";
		$query = Executor::doit($sql);
		$array = array();
		$cnt = 0;
		while($r = $query[0]->fetch_array()){
			$array[$cnt] = new tbOrganizacionData();
			$array[$cnt]->id = $r['idOrg'];
			$array[$cnt]->name = $r['nombOrg'];
			$cnt++;
		}
		return $array;
	}
