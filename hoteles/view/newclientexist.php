...
if(!empty($_POST["clientemail"])) {
  ...
  if($rowcount>0){
	...
	require_once('d:\xampp_new\htdocs\turismo\vendor\phpmailer\phpmailer\src\Exception.php');
	require_once('d:\xampp_new\htdocs\turismo\vendor\phpmailer\phpmailer\src\PHPMailer.php');
	require_once('d:\xampp_new\htdocs\turismo\vendor\phpmailer\phpmailer\src\SMTP.php');
   // print "<script>window.location='index.php?view=clientnew.php&client=" . $_POST['clientemail'] . "';</script>";
	...
	$mailContent .= 'http://localhost/turismo/hoteles/index.php?view=newclientdocs.php';
	...
	if($phpmailer->send()){
		...
		swal.fire({
	        ...
	        if (result.isConfirmed) {
	            window.location="index.php?view=newportada.php"
	        };
	    })
		</script>';
	}else{
