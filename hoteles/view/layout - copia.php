...
<body>
	...
	<?php 
		...
		if($_GET['view']=="clienthome.php"){
			include_once("clientmenu.php");
		}else{
			include_once("modgeneral/modbusca1.php");	
		}
		if(!isset($_GET['view'])){
			include "core/modules/general/view/portada.php";
		}else{
			$valid=false;
			if(file_exists($file = "core/modules/general/view/".$_GET['view']) && $_GET['view']!="client"){
				include "core/modules/general/view/".$_GET['view'];				
			}elseif($_GET['view']!="client"){ ?>
				<div style="float:right;width:87%">
					<?php include "core/modules/general/view/".$_GET['view']; ?>
				</div>
			<?php }else{
