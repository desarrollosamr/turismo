<?php
  ...
  function valida($m){
    $sw = 0;
    $sw = (empty(trim($m['nomocupaciones']))) ? 2  : $sw;
    return $sw;
  }
	if (!empty($_POST))  
	{ //$datosRecibidos="";
    ...
    switch($action){       
      case 'new' :
        ...
        $obj = new clsocupaciones();
        if ($swok == 0) {  $swok = $obj->newOcupaciones($_REQUEST); }
        echo json_encode($swok, JSON_UNESCAPED_UNICODE);      
        break;      
        //$obj = new clsmobiliario();
        //echo json_encode($obj->newOcupaciones($_REQUEST), JSON_UNESCAPED_UNICODE);        
        //break;
      case 'saveedit':
        $swok = valida($_REQUEST);
        $obj = new clsocupaciones();  
        if ($swok == 0) {$swok = $obj->updateOcupaciones($_REQUEST);}      
        echo json_encode($swok, JSON_UNESCAPED_UNICODE);
        break;
      case 'delete' :
        $obj = new clsocupaciones();
        echo json_encode($obj->oneOcupaciones($_REQUEST['action']), JSON_UNESCAPED_UNICODE);
        //$m = $_REQUEST;
      ...
      case 'savedelete':
        ...
        if ($swok == 0){
          $swok = $obj->newOcupaciones(trim($_REQUEST['nomMobiliario']), 
