...
<?php endif;?>
		<h1>SIPUVE - Sistema de Información para Puntos de Ventas</h1>
...
if($page==1){
	if(isset($_GET["product"])){
		$curr_products = ProductData::getAllByPage($products[0]->id,$limit,$_GET["product"]);
	} else {		
		$curr_products = ProductData::getAllByPage($products[0]->id,$limit,"");
	}
}else{
	if(isset($_GET["product"])){
		$curr_products = ProductData::getAllByPage($products[($page-1)*$limit]->id,$limit,$_GET["product"]);
	} else {		
