<?php
  ...
  $objMedio = new clsmedio();
  //$cantidad = $objMedio->listado(2, 'ce', $pagina, $registro); // Contar cuantos hay

  $cantidad = $objMedio->listado(2, $criterio, $pagina, $registro); // Contar cuantos hay
  /*
...
die();
  */
  if ($cantidad>0){
    ...
    $array = $objMedio->listado(1, $criterio, (($pagina - 1) * $registro), $registro);
    //if($criterio=="ce"){
    ...
    <table class='table display table-striped table-bordered table-hover' style='width:100%' cellspacing='0'>
      ...
      <tbody>"; 
        foreach ($array as $value) {
          $srttable.= "
          <tr id='row" .$value['idmedio']."'>        
            <td id='nom".$value['idmedio']."'>"   .$value['nomMedio']. "</td>
            <td class='centro'>
              <a href='javascript:void(0);' class='btn btn-warning btn-sm fa fa-edit edit' onclick='editatype(".$value['idmedio'].")'title='Editar' type='button'></a>
            </td>
            <td class='centro'> 
              <a href='javascript:void(0);' class='btn btn-danger btn-sm fa fa-trash-o delete' onclick='deletetype(".$value['idmedio'].")'title='Editar' type='button'></a>        
            </td>
          </tr>";
        }    
        $srttable.="
