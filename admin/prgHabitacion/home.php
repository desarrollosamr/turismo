      ...
      <div class="container-fluid">
        <div class="row" style="background: #E1B3B3;">
          <div class="col-md-5 col-sm-5">
            <h1 class="page-header" style="border-bottom: 0px; margin-bottom:5px;">Habitaciones <small></small></h1>
          </div>
                  ...
                  <div class="panel-heading">
                    <h4 class="panel-title">
                      <a href="#" type="button" class="btn btn-success btnew">
                        <i class="fa fa-plus fa-spin"> </i> Crear habitaci&oacute;n
          ...
          <form autocomplete="off" id="frm" action="#">
            ...
            <div class="form-row">
              <div class="form-group col-md-3 col-sm-3">
                <label for="nm">N&uacute;mero</label>
                <input id="nroroom" name="nroroom" class="form-control" type="text" placeholder="N&uacute;mero habitaci&oacute;n" required>
              </div>
              <div class="form-group col-md-9 col-sm-9">
                ...
                $rs = $objrs->fillcbostateHabitacion();
                echo $rs;
                ?>
              </div>
            </div>
            <div class="form-row">
              <div class="form-group col-md-4">
                ...
                <input id="priceroom" name="priceroom" class="form-control" type="text" placeholder="Precio habitaci&oacute;n" required>
              </div>
              <div class="form-group col-md-8">
                <label for="nm">Tipo</label>
                <?php
                include_once("../modpublicos/conexion.php");
                $objrs = new clsHabitacion();
                $rs = $objrs->fillcbotypeHabitacion();
                echo $rs;
                ?>
              </div>
            </div>
  ...
  <script type="text/javascript">
    ...
    */
    ...
    $(".btnew").click(function(e) {
      ...
      $("#titModal").html("Nuevo " + "<span>Habitaci&oacute;n</span>"); //+ id);
      ...
      switch ($("#id").val()) {
        ...
        case '<?php echo openssl_encrypt("new", COD, KEY); ?>':
          doAjax(function(serverResponse) {
            ...
            switch (serverResponse) {
              case '1':
                mysweet("Error: Numero no valido o ya registrado", "error");
                break;
              case '2':
                mysweet("Error: Seleccione estado", "error");
                break;
              case '3':
                mysweet("Error: Precio no valido estado", "error");
                break;
              case '4':
                mysweet("Error: Seleccione tipo", "error");
                break;
              case '5':
                $('#btcerrar').click();
                buscarobjMunicipios('', npi, npf);
                mysweet("Exito: Habitacion almacenada", "success");
                break;
            }
          });
          break;
    ...
    function fillrow(id) {
      $('#nom' + id).html($('#namestateh').val());
    }

    function editatype(id) {
      $("#titModal").html("Edici&oacute;n " + "<span>Habitaci&oacute;n</span>"); //+ id);
      $('input').prop('disabled', false);
      fillformulario(id);
